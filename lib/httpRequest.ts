...
async  function getDatas<T,Q>(path:string,query?:Q,secured:boolean=true) {
 // const [cookies, setCookie] = useCookies(['user']);
  ...
  try {
    ...
    const { data } = await axios.get<GetListResponse<T>>(
      url,
      {
        headers: header,
        params:query,
...
async  function getData<T>(path:string,secured:boolean=true) {
  ...
  try {
    ...
    const { data } = await axios.get<GetSingleResponse<T>>(
      url,
      {
        headers: header,
...
console.log(header)
  try {
    ...
    const { data } = await axios.post<GetSaveResponse>(
      url,
      postdata,
      {
        headers: header,
...
async  function patchData<T>(path:string,postdata:T,secured:boolean=true) {
  let url= config.serverURI+path
  let token =getToken()
  let header:RawAxiosRequestHeaders= {
    Accept: 'application/json'
  ...
  try {
    ...
    const { data } = await axios.patch<GetSaveResponse>(
      url,
      postdata,
      {
        headers: header,
...
console.log(header)
  try {
    ...
    const { data } = await axios.delete<GetSaveResponse>(
      url,
      {
        headers: header,
      },
...
async  function getDataAll<T,Q>(path:string,query?:Q,secured:boolean=true) {
  let url= config.serverURI+path
  let token =getToken()
  try {
    let header:RawAxiosRequestHeaders= {
    ...
    const { data } = await axios.get<GetResponse<T>>(
      url,
      {
        headers: header,
        params:query
...
async  function getDataSuggestions<T>(path:string,keyword:string,secured:boolean=true) {
  let url= config.serverURI+path
  let token =getToken()
  let datas:Keyword = {
    search:keyword
...
async  function postLoginData(path:string,postdata:LoginData) {
  ...
  try {
    ...
    const { data } = await axios.post<GetLoginResponse>(
      url,
      postdata,
      {
        headers: header,
...
export { deleteData, getData, getDataAll, getDataSuggestions, getDatas, patchData, postData, postLoginData };
