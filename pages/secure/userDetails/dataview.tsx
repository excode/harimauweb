    ...
    import {UserDetails,UserDetailsQuery, UserDetailsService } from '@services/UserDetails';
    import { LayoutType, SortOrderType } from '../../../types/types';
    import { SortType } from '@services/CommonTypes';
    
    
    const UserDetailsDataview = () => {
        ...
        const [userDetailssBackup, setUserDetailssBackup] = useState<UserDetails[]>([]);
        ...
        const onFilter = (e: React.ChangeEvent<HTMLInputElement>) => {
            ...
            if (value.length === 0) {
                setUserDetailss(userDetailssBackup)
            } else {
                const filtered = userDetailss!.filter((userDetails:UserDetails) => {
                    return (userDetails.address.toLowerCase().includes(value)) ;
                });
        ...
        const dataviewListItem = (data: UserDetails) => {
            return (
                <div className="col-12">
                    <div className="flex flex-column md:flex-row align-items-center p-3 w-full">
                    {(data.photo!=undefined  ) 
                      ?  <img src={`${config.serverURI}/${data.photo}`} alt={data.address} className="my-4 md:my-0 w-9 md:w-10rem shadow-2 mr-5" />
                      :  <img src={"/photo_na.png"} alt={data.address} className="my-4 md:my-0 w-9 md:w-10rem shadow-2 mr-5" />
                    }
                        <div className="flex-1 flex flex-column align-items-center text-center md:text-left">
                            <div className="font-bold text-2xl">{data.address}</div>
        ...
        const dataviewGridItem = (data: UserDetails) => {
            return (
                <div className="col-12 lg:col-4">
                    <div className="card m-3 border-1 surface-border">
                      ...
                      :  <img src={`/photo_na.png`} alt={data.address} className="my-4 md:my-0 w-9 md:w-10rem shadow-2 mr-5" />
                            ...
                            <div className="text-2xl font-bold">{data.address}</div>
        ...
        const itemTemplate = (data: UserDetails, layout: LayoutType) => {
            if (!data) {
                return;
            }
    ...
    export default UserDetailsDataview;
